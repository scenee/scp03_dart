...
void main() {
  group("TLV", () {
    test("Parse short tag and length", () {
      final tlvs = TLV.parse([0x59, 0x02, 0x95, 0x02]);
    ...
    test("Parse long tag and short length", () {
      final tlvs = TLV.parse([0x5F, 0x24, 0x03, 0x97, 0x03, 0x31]);
      expect(tlvs.length, equals(1));
    ...
    test("Parse multiple TLVs", () {
      final tlvs = TLV
          .parse([0x59, 0x02, 0x95, 0x02, 0x5F, 0x24, 0x03, 0x97, 0x03, 0x31]);
    ...
    test("Parse long tag and long length", () {
      final tlvs = TLV
          .parse([0x5F, 0x81, 0x24, 0x82, 0x01, 0x00] + List.filled(256, 0x97));
      expect(tlvs.length, equals(1));
    ...
    test("Parse invalid TLV", () {
      final tlvs = TLV.parse([0x5F, 0x81, 0x24, 0x82, 0x01]);
